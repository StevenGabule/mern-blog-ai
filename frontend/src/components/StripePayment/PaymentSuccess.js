...
import { verifyPaymentAPI } from '../../api/StripePayment/StripePaymentAPI';

const PaymentSuccess = () => {
  const [searchParams] = useSearchParams()
  const paymentIntentID = searchParams.get('payment_intent');
  const {isLoading, isError, data, isPending, isSuccess} = useQuery({queryFn: () => verifyPaymentAPI(paymentIntentID)});
  return (
